...
function renderCountdown() {
  ...
  if (!remaining) {
    target.textContent = "새로운 여름 세일 상품으로 교체 중입니다.";
  ...
  target.textContent = `상품 교체까지 ${days}일 ${String(hours).padStart(2, "0")}시간 ${String(minutes).padStart(2, "0")}분 ${String(seconds).padStart(2, "0")}초`;
}
...
async function loadSale() {
  ...
  }).join("") : `<div class="sale-loading">현재 준비된 여름 세일 상품이 없습니다.</div>`;
  clearInterval(refreshTimer);
  renderCountdown();
